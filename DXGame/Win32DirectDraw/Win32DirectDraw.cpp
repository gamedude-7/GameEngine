...
int Game_Main()
{
	...
	for (vector< Object >::const_iterator objIterator = level.objectList.begin(); objIterator !=  level.objectList.end(); objIterator++)
	{
        ...
        {                 				
			...
			{				
				Draw_Line16( pts[objIterator->plist[poly].vert[0]].getX(), pts[objIterator->plist[poly].vert[0]].getY(), pts[objIterator->plist[poly].vert[1]].getX(), pts[objIterator->plist[poly].vert[1]].getY(), RGB16Bit565(255,0,0), back_buffer, ddsd.lPitch);				
			}
			else
			{
				continue;
			...
			{
				Draw_Line16( pts[objIterator->plist[poly].vert[1]].getX(), pts[objIterator->plist[poly].vert[1]].getY(), pts[objIterator->plist[poly].vert[2]].getX(), pts[objIterator->plist[poly].vert[2]].getY(), RGB16Bit565(0,255,0), back_buffer, ddsd.lPitch);
			}	
			else
			{
				continue;
			...
			if (pts[objIterator->plist[poly].vert[2]].getX() > 0 && pts[objIterator->plist[poly].vert[2]].getX() < SCREEN_WIDTH &&
				...
				pts[objIterator->plist[poly].vert[0]].getY() > 0 && pts[objIterator->plist[poly].vert[0]].getY() < SCREEN_HEIGHT)
			{
				Draw_Line16( pts[objIterator->plist[poly].vert[2]].getX(), pts[objIterator->plist[poly].vert[2]].getY(), pts[objIterator->plist[poly].vert[0]].getX(), pts[objIterator->plist[poly].vert[0]].getY(), RGB16Bit565(0,0,255), back_buffer, ddsd.lPitch);
			}
			else
			{
				continue;
