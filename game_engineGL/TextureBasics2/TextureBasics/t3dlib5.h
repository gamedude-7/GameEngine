...
typedef struct OBJECT4DV1_TYP
{
	...
	OBJECT4DV1_TYP() {};
    OBJECT4DV1_TYP(const OBJECT4DV1_TYP &rhs)
	{
		id = rhs.id;	   
	   strcpy(name,rhs.name);
	   state = rhs.state;
	   attr = rhs.attr;
	   avg_radius = rhs.avg_radius;
	   max_radius = rhs.max_radius;
	   ...
	   for (int i=0; i<OBJECT4DV1_MAX_VERTICES;i++)
	   {
		   vlist_local[i] = rhs.vlist_local[i];
		   vlist_trans[i] = rhs.vlist_trans[i];
	   }
	   num_polys = rhs.num_polys;
	   for (int j=0; j<num_polys;j++)
	   {
		   for (int k=0; k<3;k++)
			plist[j].vert[k] = rhs.plist[j].vert[k];
	   }
	}

	OBJECT4DV1_TYP &operator=(const OBJECT4DV1_TYP &rhs) 
	{
	   id = rhs.id;	   
	   strcpy(name,rhs.name);
	   state = rhs.state;
