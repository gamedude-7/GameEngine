...
vector<Vector> vlist; // the vertex list itself
...
//    unsigned char* buffer;           // this is a pointer to the data
//
...
int Draw_Pixel16(int x, int y, int color, unsigned char* video_buffer, int lpitch);
int Draw_Line16(int xo, int yo, int x1, int y1, int color, unsigned char* vb_start, int lpitch);
...
int Close_Error_File(FILE* fp_error);
int Write_Error(FILE* fp_error, char* string, ...);
